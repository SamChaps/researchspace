...
export class SearchSummary extends Component<Props, {}> {
  ...
  render() {
    return D.div(
        ...
        D.span({className: styles.start}, this.props.search ? SearchSummary.VOCABULARY.SUMMARY_PREFIX : SearchSummary.VOCABULARY.EMPTY),
  ...
  private htmlSummary(search: Model.Search): Array<ReactElement<any>> {
    return this.searchSummary(search)
      .map(
        statement =>
          D.span(
            {className: classNames(styles.word, statement.className)},
            statement.text
          )
      );
  }
  ...
  private domainText = (domain: Model.Category): ChunkText => {
    const domainText =
        nlp.noun(domain.label).pluralize();
    return {
  ...
  private clausesText = (clauses: Model.Conjuncts): Array<ChunkText> => {
    return _.dropRight(
      _(clauses).map(this.clauseText).reduce(
        this.reduceClauses, []
      ), 1
    );
  }
  ...
  private reduceClauses = (list: Array<ChunkText>, clause: ConjunctText): Array<ChunkText> => {
    const disjuncts = _(clause.disjuncts).map(
      term => [term, this.orSeparator()]
    ).flatten<ChunkText>().dropRight(1).value();
    return list.concat(
      clause.chunks, disjuncts, [this.andSeparator()]
    );
  }
  ...
  private disjunctsText = (disjunct: Model.Disjuncts): Array<DisjunctText> =>
    _.map(disjunct, this.disjunctText)
